...
var clusterNodeImageMap = make(map[string]string)
...
func (rs *ResourceServiceImpl) InputWorkflowTask(ctx context.Context,request *TaskContainerBuilder.InputWorkflowTaskRequest)(*TaskContainerBuilder.InputWorkflowTaskResponse,error) {
	...
	for labelKey, labelValue := range request.Labels {
		...
		if _, ok := m[labelValue]; !ok {
			...
			clusterNodeImageMap[labelValue] = imageAddress
			m[labelValue] = true
		}
	}
	log.Println(clusterNodeLabelMap)
	log.Println(clusterNodeImageMap)
...
func DeleteCurrentFailedTaskContainer(param interface{})(*TaskContainerBuilder.InputWorkflowTaskResponse,error) {
	...
	if taskName, ok := (param).(string); ok {
		...
		task.Labels, task.Image = VerticalTaskOffloadingScheme(task)
...
func VerticalTaskOffloadingScheme(task WorkflowTask) (labels map[string]string, image string) {

	for labelKey, labelValue := range task.Labels {

		//Obtain the map of remaining resources of each node
		splitLabelKey :=  strings.Split(labelKey,"-")
		...
		cpuTemp := uint64(0)
		memTemp := uint64(0)
		for nodeLabelKey, _ := range clusterNodeLabelMap {
			splitNodeLabelKey := strings.Split(nodeLabelKey, "-")
			//The cloud node has the same index number as an edge node..
			if splitLabelKey[1] == splitNodeLabelKey[1] && splitNodeLabelKey[0] == "node" {
				log.Printf("splitNodeLabelKey: %s",splitNodeLabelKey)
				for _, iPAddress := range clusterNodeLabelMap[nodeLabelKey] {
					if labelValue != iPAddress {
						if ResidualMap[iPAddress].MilliCpu >= cpuTemp && ResidualMap[iPAddress].Memory >= memTemp {
							...
							task.Labels[nodeLabelKey] = iPAddress
							task.Image = clusterNodeImageMap[iPAddress]
						}
					}
				}
			}
	...
	log.Printf("rescheduling on: %s,image: %s",task.Labels,task.Image)
	return task.Labels, task.Image
}
