...
func updateFailedTaskPodLabelAndBoolInRedis(task WorkflowTask, redisServer string, boolValue bool ){
	...
	log.Printf("update taskName: %v,taskData: %v\n",task.TaskName, unMarshalData)
	// 关闭连接
	conn.Close()

	<- redisSema
}
func updateResourceShortageTaskPodLabelAndBoolInRedis(task WorkflowTask, redisServer string){
	var unMarshalData saveWorkflowTaskData
	defer updateDataInRedisFail()
...
func HorizontalTaskTransferScheme(task WorkflowTask) (labels map[string]string) {
	...
	for labelKey, labelValue := range task.Labels {
		...
		cpuTemp := uint64(0)
		memTemp := uint64(0)
		splitName := strings.Split(labelKey, "-")
		// If cloud node suffers from resource shortage
		if splitName[0] == "node" {
			for nodeLabel, iPAddressSlice := range clusterNodeLabelMap {
				//task.Labels = make(map[string]string)
				splitNameAgain := strings.Split(nodeLabel, "-")
				if splitNameAgain[0] == "node" && labelValue != iPAddressSlice[0] {
					if ResidualMap[iPAddressSlice[0]].MilliCpu >= cpuTemp &&
						ResidualMap[iPAddressSlice[0]].Memory >= memTemp {
						cpuTemp = ResidualMap[iPAddressSlice[0]].MilliCpu
						memTemp = ResidualMap[iPAddressSlice[0]].Memory
						task.Labels = make(map[string]string)
						task.Labels[nodeLabel] = iPAddressSlice[0]
					}
				}
			}
		} else {
			for _, iPAddress := range clusterNodeLabelMap[labelKey] {
				if labelValue != iPAddress {
					if ResidualMap[iPAddress].MilliCpu >= cpuTemp &&
						ResidualMap[iPAddress].Memory >= memTemp {
...
func CloudEdgeResourceScalingAlgorithm(task WorkflowTask,redisIpPort string) (cpuNum uint64, memNum uint64) {
  ...
  for {
	  ...
	  if cpuNum >= task.MinCpu && memNum >= (task.MinMem + 20) {
		  ...
		  log.Println("task label changed because of the remaining resource shortage.")
		  //update FailedTaskPod's Label and Bool in Redis
		  updateResourceShortageTaskPodLabelAndBoolInRedis(task,redisIpPort)

		  //Obtain workflowMap and update workflowMap
		  //splitName :=  strings.Split(task.WorkflowId,"-")
		  //wfId, err := strconv.Atoi(splitName[1])
		  //if err != nil {
			//  panic(err.Error())
		  //}
		  //workflowMap[uint32(wfId)][task.TaskOrder] = task
		  //log.Println(task)
		  //log.Println(workflowMap[uint32(wfId)][task.TaskOrder])
	  }
  }
	return cpuNum, memNum
}
